...
export const PileStressChartCanvas: React.FC<PileStressChartCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    ctx.fillRect(exX + (exX > centerX ? -140 : 10), exY - 14, 130, 24);
    ctx.strokeStyle = '#475569';
    ctx.strokeRect(exX + (exX > centerX ? -140 : 10), exY - 14, 130, 24);
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 shadow-md flex flex-col items-center">
      <div className="w-full flex items-center justify-between pb-2 mb-2 border-b border-slate-800">
        ...
        <div className="flex bg-slate-800 p-0.5 rounded border border-slate-700 gap-1">
          <button
            onClick={() => setActiveMetric('moment')}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
              activeMetric === 'moment' ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            M図 (曲げ)
          </button>
          <button
            ...
            className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
              activeMetric === 'shear' ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            S図 (せん断)
          </button>
          <button
            ...
            className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
              activeMetric === 'deflection' ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            y図 (変位)
          </button>
          <button
            ...
            className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
              activeMetric === 'reaction' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            p図 (地盤反力)
          </button>
