...
import { CalculationResult, MomentCurvatureState, SectionStressCheckResult } from '../../types/calculation';
...
const momentStateLabel: Record<MomentCurvatureState, string> = {
  elastic: '弾性',
  cracked: 'ひび割れ後',
  yielded: '降伏後',
  fully_plastic: '全塑性到達',
      ...
      <div className="grid gap-3 lg:grid-cols-[210px_minmax(0,1fr)_250px] lg:items-start">
        ...
        <main className="min-w-0">
          <div className="no-print mb-2 flex items-center justify-between border border-slate-300 bg-white px-3 py-2 text-[10px] text-slate-600">
            <span>詳細計算書 / 全9章</span><span className="font-mono">A4縦・印刷倍率 100%</span>
          </div>
          <article className="mx-auto max-w-[210mm] space-y-10 bg-white px-5 py-7 text-[10px] leading-relaxed text-slate-900 shadow-xl sm:px-8 sm:py-10 print:max-w-none print:space-y-8 print:p-0 print:shadow-none">
            ...
            <ReportSection id="report-conditions" number={1} title="設計条件">
              ...
              {spec.sectionSegments?.length ? <><h3 className="font-black">1.1.1 鋼管断面区分</h3><TableWrap><table className="w-full min-w-[620px] border-collapse text-center"><thead className="bg-slate-100"><tr><th className="p-1.5">区間</th><th>杭頭からの深度 z (m)</th><th>鋼管厚 t (mm)</th><th>腐食代 (mm)</th><th>降伏強度 σy (N/mm²)</th></tr></thead><tbody>{spec.sectionSegments.map((segment) => <tr key={segment.id} className="border-t border-slate-200"><td className="p-1.5 font-bold">{segment.id}</td><td>{fmt(segment.depthTop, 3)} - {fmt(segment.depthBottom, 3)}</td><td>{fmt(segment.wallThickness, 1)}</td><td>{fmt(segment.corrosionAllowance, 1)}</td><td>{fmt(segment.steelYieldStrength, 0)}</td></tr>)}</tbody></table></TableWrap></> : null}
        ...
        <aside className="no-print space-y-3 lg:sticky lg:top-28">
          ...
          <section className="border border-slate-300 bg-white p-3 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xs font-black">章構成カバー</h2><span className="text-lg font-black text-blue-700">{coverage}%</span></div><div className="mt-2 h-2 bg-slate-200"><div className="h-full bg-blue-700" style={{ width: `${coverage}%` }} /></div><p className="mt-2 text-[9px] leading-4 text-slate-500">7章は場所打ちRC杭・鋼管杭・鋼管ソイルセメント杭の深度方向増分解析、8章は矩形等厚底版の反力包絡まで実装。PHC・SCの増分解析とテーパー・柱間底版は今後の対象です。</p></section>
