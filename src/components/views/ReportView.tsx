      ...
      <div className="grid gap-3 lg:grid-cols-[210px_minmax(0,1fr)_250px] lg:items-start">
        ...
        <main className="min-w-0">
          ...
          <article className="mx-auto max-w-[210mm] space-y-10 bg-white px-5 py-7 text-[10px] leading-relaxed text-slate-900 shadow-xl sm:px-8 sm:py-10 print:max-w-none print:space-y-8 print:p-0 print:shadow-none">
            ...
            <ReportSection id="report-l2" number={7} title="レベル2地震時の照査" subtitle="M-φ骨格・初降伏判定・等価割線荷重変位曲線">
              <div className="border-l-4 border-amber-500 bg-amber-50 p-3 text-amber-950"><div className="font-black">適用範囲</div><p className="mt-1">RC系杭はO-C-Y-Uトリリニア、鋼管系杭はMy-Mpバイリニアとし、杭ごとの死荷重時軸力で骨格を作成する。水平力と転倒モーメントを同一比率で漸増し、M-φ割線EIとChang型水平ばねのEI 1/4乗則から荷重変位曲線を推定する。地盤ばね塑性化と各増分での軸力再配分を含む厳密な全体系増分解析ではない。</p></div>
              {l2Results.map((result) => (
                <div key={result.loadCaseId} className="space-y-2">
                  ...
                  {result.loadDisplacementCurve ? <>
                    ...
                    <TableWrap><table className="w-full min-w-[620px] border-collapse text-center"><thead className="bg-slate-100"><tr><th className="p-1.5">荷重倍率</th><th>H (kN)</th><th>M (kN·m)</th><th>δ (mm)</th><th>等価剛性比</th><th>状態</th></tr></thead><tbody>{result.loadDisplacementCurve.points.map((point) => <tr key={point.loadFactor} className="border-t border-slate-200"><td>{fmt(point.loadFactor, 2)}</td><td>{fmt(point.horizontalLoad)}</td><td>{fmt(point.overturningMoment)}</td><td>{fmt(point.displacement, 3)}</td><td>{fmt(point.equivalentStiffnessRatio, 3)}</td><td>{point.state === 'elastic' ? '弾性' : point.state === 'cracked' ? 'ひび割れ後' : point.state === 'yielded' ? '降伏後' : '終局超過'}</td></tr>)}</tbody></table></TableWrap>
                  </> : null}
                </div>
              ))}
        ...
        <aside className="no-print space-y-3 lg:sticky lg:top-28">
          ...
          <section className="border border-slate-300 bg-white p-3 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xs font-black">章構成カバー</h2><span className="text-lg font-black text-blue-700">{coverage}%</span></div><div className="mt-2 h-2 bg-slate-200"><div className="h-full bg-blue-700" style={{ width: `${coverage}%` }} /></div><p className="mt-2 text-[9px] leading-4 text-slate-500">7章は等価割線荷重変位、8章は矩形等厚底版の反力包絡まで実装。厳密な全体系増分解析とテーパー・柱間底版は今後の対象です。</p></section>
          ...
          <section className="border border-red-200 bg-red-50 p-3 text-red-900 shadow-sm"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-red-600" /><div><h2 className="text-xs font-black">注意事項</h2><p className="mt-1 text-[10px] leading-4">荷重変位は等価割線推定、底版は矩形等厚断面の簡易詳細照査です。正式設計では基準版、材料構成則、地盤ばね塑性化、軸力再配分、テーパー・柱間断面を別途確認してください。</p></div></div></section>
        </aside>
      </div>
    </div>
  );
