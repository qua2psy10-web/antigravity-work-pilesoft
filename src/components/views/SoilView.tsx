      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 shadow-lg">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">地下水位 (GL-m)</label>
              <input
                type="number"
                step="0.1"
                value={ground.groundWaterLevel}
                onChange={(e) => onChangeGround({ ...ground, groundWaterLevel: parseFloat(e.target.value) || 0 })}
                className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-slate-100 focus:outline-none focus:border-blue-500 font-mono"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">地盤種別</label>
              <select
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">L1 水平震度 kh</label>
              <input
                type="number"
                step="0.05"
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">L2 TypeI 震度 khgL</label>
              <input
                type="number"
                step="0.1"
                ...
                {ground.layers.map((layer, index) => {
                  ...
                  return (
                    <tr key={layer.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-2 px-2 text-slate-400 font-mono">{index + 1}</td>
                      <td className="py-2 px-2">
                        <input
                          type="text"
                          value={layer.name}
                          onChange={(e) => handleUpdateLayer(index, { name: e.target.value })}
                          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-200 text-xs w-28 focus:outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="py-2 px-2">
                        <select
                          ...
                          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-200 text-xs focus:outline-none focus:border-blue-500"
                        >
                      ...
                      <td className="py-2 px-2">
                        <input
                          ...
                          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-200 text-xs w-16 font-mono focus:outline-none focus:border-blue-500"
                        />
                      </td>
                      ...
                      <td className="py-2 px-2">
                        <input
                          ...
                          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-cyan-300 font-bold text-xs w-14 font-mono focus:outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="py-2 px-2">
                        <input
                          ...
                          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-200 text-xs w-14 font-mono focus:outline-none focus:border-blue-500"
                        />
                      </td>
        ...
        {liqResults.some((r) => r.isLiquefied) && (
          <div className="bg-red-950/40 border border-red-800/80 rounded-lg p-4 flex items-start gap-3 text-sm text-red-200">
            <ShieldAlert className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
            <div>
              <div className="font-bold text-red-300">地震時 液状化判定警告 (FL &lt; 1.0)</div>
              <div className="text-xs text-red-300/80 mt-1">
                一部の土層で液状化の可能性が判定されました。地震時解析において地盤反力低減係数 De（0〜2/3）が自動適用されます。
              </div>
            </div>
          </div>
        )}
